...
\begin{enumerate}
  ...
  полученным номером № = номер варианта * 11 * год рождения * день * месяц.
  \item Граничный условия предложить самостоятельно (торроидальные, нулевые, единичные). 
  ...
  \item Реализация возможна в консоли или в графике. 
...
где номер варианта = 11. Получилось значение 2'424'840, которое далее было представлено в двоичном
виде и дополнено до 32 символов незначащими ведущими нулями. Далее использовался  получившийся 
вектор-столбец 
$$f(s_0, s_1, s_2, s_3, s_4) = (00000000001001010000000000001000),$$
...
\begin{enumerate}
  ...
  полученным номером № = номер варианта * 11 * год рождения * день * месяц.
  \item Граничные условия выбирает пользователь (торроидальные, нулевые, единичные). 
  \item Пользователь определяет ширину поля и количество итераций. 
  \item Учтена возможность ввода различных начальных условий (как вручную, так и 
  случайным образом) по выбору пользователя. 
  \item Реализация в консоли.
  \item Проанализирован клеточный автомат в отчете.
\end{enumerate}

В  итоге получился клеточный автомат, реализующий правило 2'424'840 с четырьмя соседями (окрестность фон Неймана). 
...
\begin{itemize}
  ...
  \item Сходится к периодичности состояний.
...
\begin{itemize}
  ...
  \item Сходится к состоянию исчезновения всех живых клеток.
